...
class ConditionalDataset(torch.utils.data.Dataset):
    def __init__(self, paths):
        ...
        for fname in self.filenames:
            ...
            spec_filename = f'{fname}.spec.npy'
            spectrogram = np.load(spec_filename)
            self.audio_list.append(signal[0])
            self.spectrogram_list.append(spectrogram.T)
            _, ffname = os.path.split(fname)
            label_tmp = ffname.split('.')[0]
            label = label_tmp[-1]
    ...
    def __getitem__(self, idx):
        return {
            ...
            'spectrogram': self.spectrogram_list[idx]
        }
...
class Collator:
  ...
  def collate(self, minibatch):
    ...
    label = np.stack([record['label'] for record in minibatch if 'label' in record])
    if self.params.unconditional:
        return {
